...
namespace SerialPortLib
{
    public class SerialPortHelper
    {
        ...
        public bool SendProtocolData(ISerialPortProtocol protocol, params string[] datas)
        {
            try
            {
                var sendData = GetProtocolData(protocol.cmd, protocol.start, protocol.split, protocol.end, protocol.checksum, datas);
        ...
        public byte[] GetProtocolData(string cmd, string start, string split, string end, string checksum,params string[] datas)
        {
            var data = $"{start}{cmd}{split}{datas.ToLongString("abc")}{split}{checksum}{end}";
            data = GetASCIIString(data);
        ...
        public ISerialPortProtocol ResolveCommand(string str)
        {
            ...
            try
            {
                ...
                end = str.Substring(str.Length - 2, 2);
            }
            catch
            {
            ...
            return new CommonProtocol() { cmd = cmd, checksum = checksum, data = data, end = end, split = split, start = start };
        }
